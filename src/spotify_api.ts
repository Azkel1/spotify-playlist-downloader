  ...
  return {
    ...
    async get_playlist_song_titles(playlist_id) {
      ...
      return parsed_response.output.items.map(({ track }) => {
        const artists = track.artists.reduce((artists_names, artist, index) => {
...
export function get_access_token(client_id: string, client_secret: string) {
  ...
  const { promise, resolve, reject } = Promise.withResolvers<
    SpotifyAuth
  >();
  const server = Deno.serve({ port: SERVER_PORT }, hono.fetch);
  ...
  try {
    ...
    hono.get("/callback", async (c) => {
      ...
      if (error) {
        ...
        return c.text(`Error while authorizating Spotify API: '${error}'`);
      }
      ...
      if (code === undefined) {
        ...
        return c.text("Spotify response did not contain authorization code");
      }
      ...
      if (parsed_response.success) {
        resolve(parsed_response.output);
        return c.text(
    ...
    hono.get("/", (c) => {
      ...
      setTimeout(
        () =>
          reject("Got no response after 15s, check your browser for errors"),
        15000,
      );
    ...
    promise.finally(server.shutdown);
  } catch (err) {
    console.error(err);
    server.shutdown();
  }
